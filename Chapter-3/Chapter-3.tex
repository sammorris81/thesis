...
Finally, we present a simulation study in \sref{rbs:sim}, and a data analysis on \emph{Tamarix ramosissima} in \sref{rbs:dataanalysis}.
...
We compare our method to the spatial probit and logit for mapping the probability of the occurrence of \emph{Tamarix ramosissima} (TR) and \emph{Hedysarum scoparium} (HS), two  plant species, for a 1-km$^2$ study region of PR China \citep{Smith2012}.
...
\tref{rbtbl:dataresults} give summary Brier scores ($\times 100$) and AUROC for the \emph{Tamarix ramosissima}.
\fref{rbfig:dataroc} gives the vertically averaged ROC curves for each method and sampling setting.
