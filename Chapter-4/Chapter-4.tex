...
In this paper we propose an empirical basis function (EBF) approach to model spatially dependence extreme events that builds on a finite truncation of the spectral representation, and develops a method-of-moments estimator for the underlying spatial processes.
...
Max-stable processes have generalized extreme value (GEV; see \aref{eba:GEV}) marginal distributions.
...
\Citet[Chapter 9]{deHaan2006} show that any max-stable process can be written as
...
The higher order EBFs appear to pick up different national forest units and managed longleaf pine areas.  
...
This suggests that the 10-year return level for wildfires in regions with larger tree cover is increasing over time.
...
The data are yearly maximum of the daily precipitation totals (mm/h) of climate model output from the North American Regional Climate Change Assessment Program (NARCCAP).
This data consists of $n_s = 697$ grid cells at a 50km resolution in the eastern US, and includes simulated data under current conditions (1969 -- 2000) as well as future conditions (2039 -- 2070).
Because the data are block maxima, we set $T = -\infty$.
Plots of the time series for the current and future conditions are given in \fref{ebfig:tsprecip}

\begin{figure}[htbp]  % markdown/precipitation/cv-setup.R
  \centering
  \includegraphics[width=\linewidth]{plots/precip-ts}
  \caption{Time series of yearly max precipitation (mm/h) for current (1969 -- 2000) (left) and future (2039 -- 2070) (right) for 50 randomly selected grid cells.}
...
\end{figure}
As a comparison, we provide the first six principal components of the precipitation data along with the cumulative sum of the first 25 eigenvalues in \aref{eba:pca}.
For the precipitation data, we run the MCMC for 25,000 iterations using a burnin period of 15,000 iterations.
We consider models fit with both EBF and GSK, and fit the model using $L = 5, 10, \ldots, 40$.
Timing for each setting of $L$ is given in \tref{ebtbl:precip-scores} for 1,000 iterations.
...
This is in contrast to the fire analysis, and is likely due to the fact that the spatial dependence is estimated to be approximately three times stronger than for the fire analysis.
...
The results seem to suggest that the strength of extreme rain events will increase between 2000 and 2070 as well as greater variability in the northeast region of the U.S. as well as Ohio and parts of the south.
